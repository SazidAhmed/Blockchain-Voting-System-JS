...
async function testAuditLogging() {
  ...
  try {
    ...
    await pool.query("DELETE FROM audit_logs WHERE event_type LIKE ?", [
      "%TEST%",
    ]);
    ...
    await pool.query(
      "UPDATE voter_registrations SET status = ? WHERE user_id = ? AND election_id = ?",
    ...
    try {
      await axios.post(
        `${API_BASE}/elections/4/vote`,
        {
          ...
          publicKey: Buffer.from(
            JSON.stringify({ kty: "EC", crv: "P-256", x: "test", y: "test" }),
          ).toString("base64"),
          timestamp: Date.now(),
        },
      ...
      console.log(
        `   Vote failed: ${error.response?.data?.message || error.message}`,
      );
    }
    ...
    if (sigLogs.length > 0) {
      ...
      console.log(
        `   Result: ${sigLogs[0].event_type.includes("FAILED") ? "FAILED" : "PASSED"}`,
      );
    } else {
