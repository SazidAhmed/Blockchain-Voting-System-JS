...
const validateRegistration = [
  body("institutionId")
    ...
    .withMessage("Institution ID must be 3-50 characters")
    .matches(/^[A-Z0-9_-]+$/i)
    .withMessage(
      "Institution ID can only contain letters, numbers, hyphens, and underscores",
    ),
  ...
  body("encryptionPublicKey")
    ...
    .trim()
    .isBase64()
    .withMessage("Encryption public key must be base64 encoded")
    .isLength({ min: 64, max: 2048 })
    .withMessage("Invalid encryption public key length"),
 ...
 */
const validateVote = [
  param("id").isInt({ min: 1 }).withMessage("Invalid election ID"),
  ...
  body("encryptedBallot")
    ...
    .trim()
    .notEmpty()
    .withMessage("Encrypted ballot is required")
    .isLength({ min: 10, max: 10000 })
    .withMessage("Invalid encrypted ballot length"),

  body("nullifier")
    .optional()
    .trim()
    .notEmpty()
    .withMessage("Nullifier is required")
    .isHexadecimal()
    .withMessage("Nullifier must be hexadecimal")
    .isLength({ min: 64, max: 64 })
    .withMessage("Nullifier must be 64 characters (SHA-256)"),
...
const validateElectionId = [
  param("id").isInt({ min: 1 }).withMessage("Invalid election ID").toInt(),
...
const validateCreateElection = [
  ...
  body("candidates.*.name")
    ...
    .withMessage("Candidate name must be 2-255 characters")
    .escape(),
  ...
  body("candidates.*.description")
    ...
    .trim()
    .isLength({ max: 1000 })
    .withMessage("Candidate description too long")
    .escape(),
