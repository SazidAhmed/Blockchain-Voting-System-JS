...
class KeyManager {
  ...
  async loadUserKeys(userId, password) {
    try {
      ...
      const keypairs = await cryptoService.retrieveKeypairs(
        (userId || "").toUpperCase(),
        password,
      );
  ...
  async exportPrivateKeysForBackup(password) {
    try {
      ...
      const exported = {
        signing: {
          publicKey: await cryptoService.exportPublicKey(
            this.currentKeys.signing.publicKey,
          ),
          privateKey: await cryptoService.exportPrivateKey(
        ...
        encryption: {
          ...
          privateKey: await cryptoService.exportPrivateKey(
            this.currentKeys.encryption.privateKey,
          ),
        },
        warning:
          "KEEP THIS BACKUP SECURE! Anyone with these keys can impersonate you.",
  ...
  /**
   * Check if keys exist for a user (IndexedDB, async)
   *
   * @param {string} userId - User identifier
   * @returns {Promise<boolean>} True if keys exist
   */
  async hasStoredKeys(userId) {
    const keyData = await cryptoService.idbGet(
      `voting_keys_${(userId || "").toUpperCase()}`,
    );
    return keyData !== undefined && keyData !== null;
  }
