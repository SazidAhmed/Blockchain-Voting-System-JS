...
class CryptoService {
  ...
  // ===== IndexedDB key storage (H-05: never store keys in localStorage) =====
  idb() {
    if (this._idb) return this._idb;
    this._idb = new Promise((resolve, reject) => {
      const req = indexedDB.open("voting-keys", 1);
      req.onupgradeneeded = () => {
  ...
  async signData(data, privateKey) {
    try {
      ...
      // Convert data to bytes (canonical JSON: sorted keys for deterministic signing)
      const dataString =
  ...
  /**
   * Store keypair in IndexedDB (encrypted with password)
   *
  ...
  async storeKeypairs(keypairs, password, userId) {
    try {
      ...
      // Random salt per encryption (H-05: never derive salt from known values)
  ...
  async retrieveKeypairs(userId, password) {
    try {
      const keyData = await this.idbGet(`voting_keys_${userId}`);
      if (!keyData) {
      ...
      const keypairs = {
        signing: {
          publicKey: await this.importPublicKey(
            exported.signing.publicKey,
            "ECDSA",
            ["verify"],
          ),
          privateKey: await this.importPrivateKey(
            exported.signing.privateKey,
            "ECDSA",
            ["sign"],
          ),
        },
        encryption: {
          publicKey: await this.importPublicKey(
            exported.encryption.publicKey,
            "RSA-OAEP",
            ["encrypt"],
          ),
          privateKey: await this.importPrivateKey(
            exported.encryption.privateKey,
            "RSA-OAEP",
            ["decrypt"],
          ),
        },
  ...
  async createVotePackage(voteData, electionId, voterKeys, electionPublicKey) {
    try {
      ...
      if (electionPublicKey && this.isValidPublicKey(electionPublicKey)) {
        ...
        throw new Error(
          "No valid election public key available — cannot encrypt ballot",
        );
      }
  ...
  // Canonical JSON serialization: keys sorted for deterministic output
