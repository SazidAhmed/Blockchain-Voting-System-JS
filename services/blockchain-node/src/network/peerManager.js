...
class PeerManager extends EventEmitter {
  ...
  addPeer(nodeId, socket, peerInfo = {}) {
    ...
    this.emit("peer_connected", {
      nodeId,
      peerInfo: this.peers.get(nodeId).info,
    });
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        socket.on("error", (error) => {
          console.error(`Socket error to ${peerUrl}:`, error.message);
        });
  ...
  handleConnectionFailure(nodeId, peerUrl) {
    ...
    console.log(
      `Retrying connection to ${nodeId} in ${delay}ms (attempt ${attempts + 1})`,
    );
  ...
  handlePeerMessage(nodeId, message) {
    ...
    if (nodeId && this.peerHealth.has(nodeId)) {
  ...
  checkPeerHealth() {
    ...
    this.peers.forEach((peer, nodeId) => {
      ...
      if (timeSinceLastHeartbeat > this.heartbeatTimeout) {
        ...
        const lastHeartbeatSnapshot = health.lastHeartbeat;
