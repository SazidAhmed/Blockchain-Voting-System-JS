...
class NodeMonitor extends EventEmitter {
  ...
  getNodeStatus() {
    ...
    const averageBlockTime =
      this.blockTimes.length > 0
        ? this.blockTimes.reduce(
            (sum, bt) =>
              sum + (bt.timestamp - (this.blockTimes[0]?.timestamp || 0)),
            0,
  ...
  determineConsensusStatus(healthyPeers) {
    // Majority of total nodes (this + peers) required for consensus
    const totalNodes = process.env.TOTAL_NODES || 5;
    const consensusThreshold = Math.floor(totalNodes / 2) + 1;
  ...
  getBlockProductionMetrics() {
    ...
    for (let i = 1; i < this.blockTimes.length; i++) {
      blockTimes.push(
        this.blockTimes[i].timestamp - this.blockTimes[i - 1].timestamp,
      );
    }
    ...
    return {
      ...
      transactionPercentage:
        this.stats.transactionsProcessed > 0
          ? Math.round(
              (this.stats.votesProcessed / this.stats.transactionsProcessed) *
                100,
            )
          : 0,
