...
router.patch(
  "/api/members/:institutionId/voter",
  apiKeyAuth,
  async (req, res) => {
    try {
      ...
      console.error("set voter error:", e);
...
router.post("/api/members", apiKeyAuth, async (req, res) => {
  try {
    const { institution_id, full_name, email, role, department, year_level } =
      req.body;
    if (!institution_id || !full_name || !email || !role || !department) {
      return res.status(400).json({
        message:
          "institution_id, full_name, email, role and department are required.",
      });
    }
    ...
    if (!validRoles.includes(role))
      return res
        .status(400)
        .json({ message: "role must be student, teacher or staff." });
    const db = getPool();
    const [[exists]] = await db.query(
      "SELECT 1 FROM institution_members WHERE institution_id = ? OR email = ?",
      [institution_id.toUpperCase(), email],
    );
    if (exists)
      return res
        .status(409)
        .json({ message: "Institution ID or email already exists." });
    await db.query(
    ...
    const [[row]] = await db.query(
      "SELECT id, institution_id, full_name, email, role, department, year_level, is_voter FROM institution_members WHERE institution_id = ?",
      [institution_id.toUpperCase()],
    );
...
router.put("/api/members/:institutionId", apiKeyAuth, async (req, res) => {
  try {
    ...
    if (!full_name || !email || !role || !department) {
      return res.status(400).json({
        message: "full_name, email, role and department are required.",
      });
    }
    const validRoles = ["student", "teacher", "staff"];
    if (!validRoles.includes(role))
...
router.delete("/api/members/:institutionId", apiKeyAuth, async (req, res) => {
  try {
    ...
    const [[row]] = await getPool().query(
      "SELECT is_voter FROM institution_members WHERE institution_id = ?",
      [id],
    );
    ...
    if (row.is_voter)
      return res
        .status(409)
        .json({ message: "Cannot delete a registered voter." });
    await getPool().query(
      "DELETE FROM institution_members WHERE institution_id = ?",
      [id],
    );
...
router.get("/api/members", apiKeyAuth, async (req, res) => {
  try {
    ...
    if (voter === "true") {
      conditions.push("is_voter = 1");
    }
    if (voter === "false") {
      conditions.push("is_voter = 0");
    }
    const where = conditions.length ? "WHERE " + conditions.join(" AND ") : "";
    const db = getPool();
    const [members] = await db.query(
      "SELECT id, institution_id, full_name, email, role, department, year_level, is_voter FROM institution_members " +
        where +
        " ORDER BY is_voter ASC, role, institution_id LIMIT ? OFFSET ?",
      [...params, limit, offset],
    );
    const [[{ total }]] = await db.query(
      "SELECT COUNT(*) AS total FROM institution_members " + where,
      params,
    );
    // Composite index idx_voter_role_id added in database.js init
    const [[counts]] = await db.query(
      "SELECT SUM(is_voter) AS voters, COUNT(*) AS total FROM institution_members",
    );
    res.json({
      members,
      ...
      stats: {
        ...
        available: counts.total - (counts.voters || 0),
      },
    });
  } catch (e) {
