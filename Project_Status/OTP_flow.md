...
The OTP system verifies a voter's email address during registration. It is implemented entirely in the backend (`backend/services/otpService.js` + `backend/services/emailService.js`) and uses an **in-memory store** (no database table required).
...
- Backend looks up the entry in the in-memory Map:
  - If not found → error: *"No verification code found. Please request a new one."*
  - If expired → entry deleted → error: *"Verification code has expired."*
  - If max attempts exceeded → entry deleted → error: *"Too many failed attempts."*
...
Environment variables required for production (in `backend/.env`):
```env
...
| File | Purpose |
|---|---|
| `backend/services/otpService.js` | OTP generation, storage, verification logic |
| `backend/services/emailService.js` | SMTP/Ethereal email sending |
| `backend/routes/users.js` | API routes: `/send-otp`, `/verify-otp`, `/register` |
| `backend/.env` | SMTP credentials configuration |
