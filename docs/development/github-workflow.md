...
> See [docs/README.md](../README.md) for full project documentation index.
...
```bash
# Backend only (requires MySQL running)
...
Tests are integration-only (no unit test framework). Run via the interactive runner:
...
# Single category (requires test Docker running)
bash tests/categories/smoke-test.sh
```

See [docs/testing/running-tests.md](../testing/running-tests.md) for full details.

---

## AI Agent Workflow

This repo uses **opencode** for AI-assisted development. Configuration:
...
- Reference `AGENTS.md` for service layout, ports, and commands
- Agent docs live in `docs/` — structured by domain
- For code changes: read existing patterns in the same service before writing
- Git hooks run automatically on commit/push — no manual setup needed

---

## Windows Notes

This repo's scripts target Linux/Mac. On Windows:

| Linux Command          | Windows Equivalent                        |
| ---------------------- | ----------------------------------------- |
| `chmod +x script.sh`   | Not needed (no permissions in PowerShell) |
| `bash scripts/ci.sh`   | `git bash scripts/ci.sh` or use WSL       |
| `cp .env.example .env` | `Copy-Item .env.example .env`             |
| `./script.sh`          | `bash script.sh`                          |

For the best experience, use **Git Bash** (included with Git for Windows) or **WSL**.
